...
use pai::{EnumIs, Furo, FuroType, IntoEnumIterator, is_chiitoi, is_kokushi, IVecPai, Mentsu, NormalMeld, Pai, ReducedFuroType, split_into_melds, Yaku};
use pai::FuroType::{AnKan, Chakan, Pon};
use pai::Yaku::{*};
...
impl YakuJudgeArgs {
        ...
        for NormalMeld { mentsu, head } in melds {
            for idx in 0..=mentsu.len() {
                if idx == mentsu.len() {
                    if machi == head {
                        rs.push(MeldDetail {
                            mentsu: mentsu.iter().cloned().map(|m| match m {
                                Mentsu::Shuntsu(p) => (MentsuDetail::Shuntsu, p),
                                Mentsu::Kotsu(p) => (MentsuDetail::Anko, p),
                            }).chain(std::iter::once((MentsuDetail::Toitsu, head))).collect(),
                            is_bad_shape: true,
                        });
                    }
                } else {
                    ...
                    if target.include(machi) {
                        rs.push(MeldDetail {
                            mentsu: mentsu.iter().cloned().enumerate().map(|(i, m)| match m {
                                Mentsu::Shuntsu(p) => (MentsuDetail::Shuntsu, p),
                                Mentsu::Kotsu(p) => (if i == idx && self.flags.is_tsumo.not() { MentsuDetail::Minko } else { MentsuDetail::Anko }, p),
                            }).chain(std::iter::once((MentsuDetail::Toitsu, head))).collect(),
                            is_bad_shape: if target.is_shuntsu() && (target.get_pai().get_number() == 1 && machi.get_number() == 3 ||
                                target.get_pai().get_number() == 7 && machi.get_number() == 7 ||
    ...
    fn judge_fu_normal(&self, meld_detail: &MeldDetail) -> (u16, bool) {
        let menzen_ron = if self.flags.is_menzen && self.flags.is_tsumo.not() { 10 } else { 0 };
        ...
        for (m, p) in meld_detail.mentsu.iter() {
            ...
            match m {
                MentsuDetail::Toitsu => {
                    if *p == self.bakaze { fu += 2; }
                    if *p == self.jikaze { fu += 2; }
                    if p.is_dragon() { fu += 2; }
                }
    ...
    /// (han, fu, tensu, yakus)
    fn judge_han_fu(&self) -> (u8, u16, u32, Vec<Yaku>) {
        let mut common_yaku_vec = vec![];
        let flags = &self.flags;
        let pais = &self.pai.iter().cloned().chain(self.furo.iter().flat_map(|f| f.pais())).collect::<Vec<_>>();

        // Special: 1-han: Akadora
        pais.iter().filter(|p| p.is_aka()).for_each(|_| common_yaku_vec.push(Akadora));
        ...
        if flags.is_tsumo {
            if flags.is_menzen {
                // Yakuman: Tenhou
                if flags.is_first && self.jikaze.is_z_1() { common_yaku_vec.push(Tenhou); }
                // Yakuman: Chihou
                if flags.is_first && self.jikaze.is_z_1().not() { common_yaku_vec.push(Chihou); }
                // 1-han: Menzen Tsumo
                common_yaku_vec.push(MenzeTsumo);
            }
            // 1-han: Rinshan Kaihou
            if flags.is_kan { common_yaku_vec.push(RinshaKaihou); }
            // 1-han: Haitei Raoyue
            if flags.is_last { common_yaku_vec.push(HaiteRaoyue); }
        } else {
            // 1-han: Chan Kan
            if flags.is_kan { common_yaku_vec.push(Chankan); }
            // 1-han: Houtei Raoyui
            if flags.is_last { common_yaku_vec.push(HouteRaoyui); }
        }
        // riichi
        // 1-han: Riichi
        if flags.is_riichi && flags.is_dbl_riichi.not() { common_yaku_vec.push(Riichi); }
        // 1-han: Double Riichi
        if flags.is_dbl_riichi { common_yaku_vec.push(DaburRiichi); }
        // 1-han: Ippatsu
        if flags.is_ippatsu { common_yaku_vec.push(Ippatsu); }
        // dora
        // 1-han: Dora
        (0..self.dora_marker.iter().map(|marker| marker.get_dora_next()).map(|dora| pais.iter().filter(|&p| p.partial_eq(&dora)).count()).sum()).for_each(|_| common_yaku_vec.push(Dora));
        if flags.is_riichi {
            // 1-han: Uradora
            (0..self.uradora_marker.iter().map(|marker| marker.get_dora_next()).map(|dora| pais.iter().filter(|&p| p.partial_eq(&dora)).count()).sum()).for_each(|_| common_yaku_vec.push(Uradora));
        }

        // all pai
        // 1-han: Tanyao
        if pais.iter().all(|p| p.is_terminal().not()) { common_yaku_vec.push(Tanyao); }
        if pais.iter().all(|p| p.is_terminal() && p.is_number()) {
            // Yakuman: Chinroutou
            common_yaku_vec.push(Chinroutou);
            // 2-han: Honroutou
        } else if pais.iter().all(|p| p.is_terminal()) { common_yaku_vec.push(Honroutou); }
        // Yakuman: Tsuuiisou
        if pais.iter().all(|p| p.is_number().not()) { common_yaku_vec.push(Tsuuiisou); }
        let first_pai = pais.first().unwrap();
        if pais.iter().all(|p| p.get_suit().eq(&first_pai.get_suit())) {
            let pai_map = pais.to_pai_map();
            if pai_map.iter().all(|(p, &cnt)| {
                if p.is_terminal() { cnt >= 3 } else { cnt >= 1 }
            }) {
                // Yakuman: Chuure Pouto
                if pai_map.get(&self.machi) % 2 == 0 { common_yaku_vec.push(ChuurePout9Wait); } else { common_yaku_vec.push(ChuurePouto); }
            } else {
                // 6-han: Chinitsu
                common_yaku_vec.push(Chinitsu);
            }
            // 3-han: Honitsu
        } else if pais.iter().all(|p| p.get_suit().eq(&first_pai.get_suit()) || p.is_number().not()) { common_yaku_vec.push(Honitsu); }
        // Yakuman: Ryuuiisou
        if pais.iter().all(|p| [Pai::S2, Pai::S3, Pai::S4, Pai::S6, Pai::S8, Pai::Z6].contains(p)) { common_yaku_vec.push(Ryuuiisou); }
        ...
        if is_kokushi(&pais) {
            ...
            for md in self.split_melds_machi_normal(split_into_melds(self.pai.iter().map(|p| p.remove_aka()).collect::<Vec<_>>())).iter() {
                let mut this_yakus = vec![];
                let (possible_fu, is_pinfu) = self.judge_fu_normal(md);
                // 1-han: Pinfu
                if is_pinfu { this_yakus.push(Pinfu); }
                possible_fus.push(possible_fu);
                if md.mentsu.iter().any(|(m, p)| m.is_kotsu() && p.eq(&self.jikaze)) {
                    if self.jikaze.is_z_1() { this_yakus.push(JikazeTon); }
                    if self.jikaze.is_z_2() { this_yakus.push(JikazeNan); }
                    if self.jikaze.is_z_3() { this_yakus.push(JikazeXia); }
                    if self.jikaze.is_z_4() { this_yakus.push(JikazeXia) }
                }
                if md.mentsu.iter().any(|(m, p)| m.is_kotsu() && p.eq(&self.bakaze)) {
                    if self.bakaze.is_z_1() { this_yakus.push(BakazeTon); }
                    if self.bakaze.is_z_2() { this_yakus.push(BakazeNan); }
                    if self.bakaze.is_z_3() { this_yakus.push(BakazeXia); }
                    if self.bakaze.is_z_4() { this_yakus.push(BakazePei); }
                }
                md.mentsu.iter().for_each(|(m, p)| {
                    if m.is_kotsu().not() { return; }
                    match p {
                        Pai::Z5 => { this_yakus.push(Haku); }
                        Pai::Z6 => { this_yakus.push(Hatsu); }
                        Pai::Z7 => { this_yakus.push(Chun); }
                        _ => {}
                    }
                });
                ...
                let toitsu = md.mentsu.iter().find(|(m, _)| m.is_toitsu()).unwrap().1;
                for (m, p) in md.mentsu.iter() {
                    match m {
                        MentsuDetail::Toitsu => {}
                        MentsuDetail::Shuntsu => {
                            shuntsu_map[p.get_suit().to_idx() as usize][p.get_number() as usize] += 1;
                        }
                        MentsuDetail::Anko | MentsuDetail::Minko | MentsuDetail::Ankan | MentsuDetail::Minkan => {
                ...
                if flags.is_menzen {
                    match shuntsu_map.iter().flatten().map(|&w| w / 2u8).sum() {
                        // 3-han: Ryanpeikou
                        2 => { this_yakus.push(Ryanpeikou); }
                        // 1-han: Iipeiko
                        1 => { this_yakus.push(Iipeiko); }
                        _ => {}
                    }
                }
                for i in 1..=9 {
                ...
                if md.mentsu.iter().all(|(m, p)| {
                    ((m.is_kotsu() || m.is_toitsu()) && p.is_terminal() ||
                        m.is_shuntsu() && p.is_number() && (p.get_number() == 1 || p.get_number() == 7)) &&
                ...
                if md.mentsu.iter().filter(|(_, p)| p.is_dragon()).count() == 3 {
                    if md.mentsu.iter().filter(|(m, p)| p.is_dragon() && m.is_kotsu()).count() == 3 {
                        // Yakuman: Daisangen
                ...
                if md.mentsu.iter().filter(|(_, p)| p.is_wind()).count() == 4 {
                    if md.mentsu.iter().filter(|(m, p)| p.is_wind() && m.is_kotsu()).count() == 4 {
                        // Yakuman: Daisuushi
        ...
        for (possible, fu) in possible_yakus.iter().zip(possible_fus) {
            let mut yakus = common_yaku_vec.iter().chain(possible.iter()).cloned().collect::<Vec<_>>();
            if yakus.iter().any(|y| y.is_yakuman()) {
                yakus = yakus.into_iter().filter(|y| y.is_yakuman()).collect();
            }
            let han = yakus.iter().map(|y| {
                let h = y.get_han();
        ...
        rs.iter().max_by(|a, b| {
            a.2.cmp(&b.2).then(a.0.cmp(&b.0)).then(a.3.iter().filter(|y| y.is_dora_type().not()).count().cmp(&b.3.iter().filter(|y| y.is_dora_type().not()).count()))
        }).unwrap().clone()
    }
}

lazy_static! {
    static ref tensu_table:Mutex<HashMap<(u8,u16),u32>> = Default::default();
}
...
fn test_yaku_judge() {
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (6, 20, 18000, vec![MenzeTsumo, Pinfu, Iipeiko, Junchan]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (1, 30, 1100, vec![MenzeTsumo]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (3, 50, 6400, vec![Tanyao, Sanankou]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![Furo { furo_type: FuroType::AnKan, target: Pai::M1, consumed: [Pai::M1, Pai::M1, Pai::M1] }],
    }.judge_han_fu(), (2, 70, 4700, vec![MenzeTsumo, Riichi]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![
            Furo { furo_type: FuroType::AnKan, target: Pai::M9, consumed: [Pai::M9, Pai::M9, Pai::M9] },
            Furo { furo_type: FuroType::Chakan, target: Pai::Z5, consumed: [Pai::Z5, Pai::Z5, Pai::Z5] },
        ],
    }.judge_han_fu(), (1, 80, 3900, vec![Haku]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (3, 40, 5200, vec![Akadora, MenzeTsumo, Riichi]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (3, 40, 7800, vec![MenzeTsumo, Riichi, Dora]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (2, 20, 1500, vec![MenzeTsumo, Pinfu]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![
            Furo { furo_type: FuroType::Chakan, target: Pai::Z6, consumed: [Pai::Z6, Pai::Z6, Pai::Z6] },
            Furo { furo_type: FuroType::Pon, target: Pai::S4, consumed: [Pai::S4, Pai::S4, Pai::Unknown] },
            Furo { furo_type: FuroType::Chi, target: Pai::S2, consumed: [Pai::S1, Pai::S3, Pai::Unknown] },
        ],
    }.judge_han_fu(), (3, 40, 5200, vec![Honitsu, Hatsu]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![
            Furo { furo_type: FuroType::AnKan, target: Pai::P1, consumed: [Pai::P1, Pai::P1, Pai::P1] },
            Furo { furo_type: FuroType::AnKan, target: Pai::P9, consumed: [Pai::P9, Pai::P9, Pai::P9] },
            Furo { furo_type: FuroType::Chakan, target: Pai::Z3, consumed: [Pai::Z3, Pai::Z3, Pai::Z3] },
        ],
    }.judge_han_fu(), (2, 110, 7100, vec![Sankantsu]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (4, 25, 6400, vec![Akadora, MenzeTsumo, Chiitoitsu]));
    ...
    }.judge_han_fu(), (3, 30, 4000, vec![Akadora, Tanyao, SanshokDoujun]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (13, 25, 48000, vec![KokushMusou]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        pai: IVecPai::from_string("19m119p19s1234567z"),
        furo: vec![],
    }.judge_han_fu(), (13, 25, 48000, vec![KokushMuso13Wait]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![],
    }.judge_han_fu(), (13, 40, 48000, vec![ChuurePouto]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        pai: IVecPai::from_string("11123406778999s"),
        furo: vec![],
    }.judge_han_fu(), (13, 40, 48000, vec![ChuurePout9Wait]));
    ...
    assert_eq!(YakuJudgeArgs {
        ...
        furo: vec![
            Furo { furo_type: Chakan, target: Pai::P1, consumed: [Pai::P1, Pai::P1, Pai::P1, ] },
            Furo { furo_type: Pon, target: Pai::Z5, consumed: [Pai::Z5, Pai::Z5, Pai::Unknown, ] },
            Furo { furo_type: AnKan, target: Pai::Z6, consumed: [Pai::Z6, Pai::Z6, Pai::Z6, ] },
        ],
    }.judge_han_fu(), (12, 90, 36000, vec![Dora, Dora, Dora, Honroutou, BakazeNan, Haku, Hatsu, Toitoi, Shousangen]));
}