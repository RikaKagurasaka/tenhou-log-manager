...
impl ShantenCalculator {
    ...
    pub fn try_load(path: &str) -> Result<Self, error::ShantenCalculatorLoadError> {
        ...
        Ok(bincode::decode_from_std_read(&mut file, bincode::config::standard())?)
    }

    pub fn calc_distance(&self, pais: &Vec<Pai>) -> u8 {
        let pais = MonoChromatic::from_pai_vec(pais);
        let mut rs = u32::MAX;
        let mentsu_cnt = ((pais.iter().map(|x| (x.size()) as f32).sum::<f32>() - 2.0) / 3.0).ceil() as u8;
    ...
    pub fn calc_shanten(&self, pais: &Vec<Pai>) -> i8 {
    ...
    pub fn calc_distance_chitoi(&self, pais: &Vec<Pai>) -> u8 {
        let pai_cnt_map = pais.to_pai_map();
        let mut cnt = [0; 5];
        for (&_pai, &c) in pai_cnt_map.iter() {
            cnt[c as usize] += 1;
        }
        let tot_pai_types = pai_cnt_map.len() as u8;
        let to_discard_over_type = if tot_pai_types > 7 { tot_pai_types - 7 } else { 0 };
        let to_discard_over_cnt = cnt[4] * 2 + cnt[3];
        let to_draw_type = if tot_pai_types < 7 { (7 - tot_pai_types) * 2 } else { 0 };
        let to_draw_cnt = cnt[1] - to_discard_over_type;
        to_discard_over_type + to_discard_over_cnt + to_draw_type + to_draw_cnt
    }

    pub fn calc_shanten_chitoi(&self, pais: &Vec<Pai>) -> i8 {
    ...
    pub fn calc_distance_gokushi(&self, pais: &Vec<Pai>) -> u8 {
        let old_len = pais.len();
        let pais = pais.iter().filter(|&x| x.is_terminal()).collect::<Vec<_>>();
        let to_discard_non_yao9 = old_len - pais.len();
        let pai_cnt_map = pais.to_pai_map();
        let types = pai_cnt_map.len();
        let has_toitsu = pais.len() > types;
        let to_discard_over_cnt = if has_toitsu { (pais.len() - 1) - types } else { 0 };
    ...
    pub fn calc_shanten_gokushi(&self, pais: &Vec<Pai>) -> i8 {
        let pais_len = pais.len();
    ...
    pub fn calc_distance_all(&self, pais: &Vec<Pai>) -> u8 {
        self.calc_distance(pais).min(self.calc_distance_chitoi(pais)).min(self.calc_distance_gokushi(pais))
    }

    pub fn calc_shanten_all(&self, pais: &Vec<Pai>) -> i8 {
        self.calc_shanten(pais).min(self.calc_shanten_chitoi(pais)).min(self.calc_shanten_gokushi(pais))
    }
}
...
fn load_and_calc() {
    ...
    println!("{:?}", pais);
    assert_eq!(calc.calc_shanten(&Vec::<Pai>::from_string("39m3p26s16z6m")), 4);
    assert_eq!(calc.calc_shanten(&Vec::<Pai>::from_string("288m29p126s46z2m")), 3);
    assert_eq!(calc.calc_shanten(&Vec::<Pai>::from_string("3689m58p2479s135z1s")), 5);
    assert_eq!(calc.calc_shanten(&Vec::<Pai>::from_string("129m12579p106s14z8p")), 3);
    assert_eq!(calc.calc_shanten(&Vec::<Pai>::from_string("23444456777998m")), 0);
    assert_eq!(calc.calc_shanten(&Vec::<Pai>::from_string("23444456677899m")), -1);
}

#[test]
fn chitoi() {
    let calc = ShantenCalculator::try_load("shanten_calculator.bin").unwrap();
    assert_eq!(calc.calc_shanten_chitoi(&Vec::<Pai>::from_string("1133m")), 4);
    assert_eq!(calc.calc_shanten_chitoi(&Vec::<Pai>::from_string("133488m669p4488s3z")), 1);
    assert_eq!(calc.calc_shanten_chitoi(&Vec::<Pai>::from_string("13388m669p4488s3z")), 1);
    assert_eq!(calc.calc_shanten_chitoi(&Vec::<Pai>::from_string("1111m2222p3333s24z")), 5);
}

#[test]
fn gokushi() {
    let calc = ShantenCalculator::try_load("shanten_calculator.bin").unwrap();
    assert_eq!(calc.calc_shanten_gokushi(&Vec::<Pai>::from_string("19m19p19s1234567z")), 0);
    assert_eq!(calc.calc_shanten_gokushi(&Vec::<Pai>::from_string("1129m89p19s1234567z")), 1);
    assert_eq!(calc.calc_shanten_gokushi(&Vec::<Pai>::from_string("123m112233p12399s")), 8);
}